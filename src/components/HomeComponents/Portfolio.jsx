...
const Portfolio = () => {
  ...
  // GSAP: scroll animation + hover enter/leave handlers
  useEffect(() => {
    ...
    return () => {
      const itemsCleanup = gsap.utils.toArray(".portfolio-item");
      itemsCleanup.forEach((item) => {
  ...
  return (
    <div
      id="projects"
      className="dark:bg-transparent transition-all duration-300 border-t-[3px] dark:border-none"
    >
      <div className="w-[90%] m-auto py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:mt-12 mt-6">
          {itemsToShow.map((item) => (
            <div
              key={item.id}
              className="portfolio-item relative bg-white dark:bg-[#0f0f2a] shadow-lg rounded-lg p-4 overflow-hidden transform transition-transform hover:scale-105 border border-gray-200 dark:border-[#333]"
            >
              {/* Updated overlay gradient */}
              <div className="overlay absolute inset-0 bg-gradient-to-t from-indigo-600/70 via-purple-500/40 to-transparent opacity-0 transition-opacity duration-300"></div>
              ...
              <p className="text-sm font-Dm_font text-gray-600 dark:text-white mt-2 z-10 relative">
                {item.client}
              </p>

              {/* action buttons */}
              <div className="action-buttons absolute inset-0 flex items-center justify-center gap-4 opacity-0 translate-y-20 z-10 pointer-events-none">
                <button
            ...
            <div className="bg-white dark:bg-[#0F1343] p-6 rounded-lg sm:w-2/3 w-full h-full shadow-lg relative">
              ...
              <div className="sticky top-0 bg-white dark:bg-[#0F1343] py-4 z-10">
                <div className="flex justify-between items-center">
                  <h3 className="font-bold text-lg">{popupData.caseStudyTitle}</h3>
                  <a
                    href={popupData.livePreview?.link}
                    target="_blank"
                    rel="noreferrer"
                    className="relative bg-[#5820FF] text-white font-inter text-sm py-2 px-4 rounded-md cursor-pointer transition-transform transform hover:scale-105 overflow-hidden group"
                  >
                    <span className="relative z-10 flex items-center gap-2">
                      <FaEye /> Live
                    </span>
              ...
              <div className="overflow-auto max-h-[calc(100vh-20vh)]">
                <img
                  src={popupData.image}
                  alt={popupData.caseStudyTitle}
                  className="w-full h-auto object-cover mt-4 rounded-md"
                />

                <div className="mt-4 text-sm text-gray-700 dark:text-gray-200">
                  <p>{popupData.client}</p>
                  <p>{popupData.location}</p>
                </div>

                <div className="mt-6">
                  <h3 className="font-inter font-medium dark:text-yellow-500">
                    Project Info:
                  </h3>
                  <ul className="list-disc pl-8 mt-1 mb-3 font-Dm_font text-sm leading-6">
                    <li>{popupData.projectInfo.pages}</li>
                    <li>{popupData.projectInfo.theme}</li>
                    <li>{popupData.projectInfo.platform}</li>
                    <li>{popupData.projectInfo.tools}</li>
                  </ul>
                </div>
                ...
                <div className="mt-6">
                  ...
                  <ul className="list-disc pl-8 mt-1 mb-3 font-Dm_font text-sm leading-6">
                    {popupData.sections.overview.map((point, idx) => (
                      <li key={idx}>{point}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="mt-6">
                  ...
                  <ul className="list-disc pl-8 mt-1 mb-3 font-Dm_font text-sm leading-6">
                    {popupData.sections.features.map((point, idx) => (
                      <li key={idx}>{point}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="mt-6">
                  ...
                  <ul className="list-disc pl-8 mt-1 mb-3 font-Dm_font text-sm leading-6">
                    {popupData.sections.valueDelivered.map((point, idx) => (
                      <li key={idx}>{point}</li>
                    ))}
                  </ul>
                </div>
        ...
        <div className="flex justify-center pt-10">
          ...
          >
            {showAll ? (
              <>
                Show Less
                <FaArrowUpLong className="group-hover:-translate-y-1 transition-transform duration-300" />
              </>
            ) : (
              <>
                Show More
                <FaArrowDownLong className="group-hover:translate-y-1 transition-transform duration-300" />
              </>
            )}
          </button>
        </div>
